...
import {FaceTrainResponseEvent, FaceTrainViewModel} from '../utils/models/face_models';
...
import {isFaceDirNameValid, toBase64} from 'src/utils/utils';
...
const $q = useQuasar();
const people = ref<FaceTrainViewModel[]>([]);
const selectedPerson = ref<FaceTrainViewModel>({name: '', image_paths: []});
...
const dataBindImages = async () => {
  const items = await nodeService.getFaceTrainPersons();
  if (items && items.length > 0) {
    for (const item of items) {
      if (item && item.image_paths && item.image_paths.length) {
        for (let j = 0; j < item.image_paths.length; ++j) {
          item.image_paths[j] = nodeService.LocalService.getServerAddressWithoutDash(item.image_paths[j]);
        }
      }
    }
...
const onTraining = () => {
  ...
  void publishService.publishFaceTrain();
};

function onSubscribeFaceTrain(event: MessageEvent) {
...
function onScreenshotComplete(event: MessageEvent) {
  ...
  nodeService.saveFaceTrainPersonImage({name: sp.name, base64_images: [responseModel.image_base64]})
    .then(() => {
...
onMounted(async () => {
  ...
  connFrTrain = subscribeService.subscribeFaceTrain(onSubscribeFaceTrain);
...
async function onLeftPersonMenuClicked(person: FaceTrainViewModel) {
  ...
  try {
    const items = await nodeService.getFaceTrainPersonImages(person.name);
    if (items && items.length > 0) {
      for (const item of items) {
...
async function onDoDelete() {
  ...
  try {
    await nodeService.deleteFaceTrainPerson({name: selectedPerson.value.name});
    mode.value = Mode.Add;
    selectedPerson.value = {name: '', image_paths: []};
    await dataBindImages();
  } finally {
...
async function onImageDelete(src: string) {
  await nodeService.deleteFaceTrainPersonImage(src);
...
async function factoryFn(files: any) {
  ...
  await nodeService.saveFaceTrainPersonImage({name: selectedPerson.value.name, base64_images: b64s});
  await dataBindImages();
  return true;
}
...
async function onRename() {
  ...
  if (!prevSelectedPersonName || !name || !isFaceDirNameValid(name)) {
    $q.notify({message: t('v_enter_valid_name'), color: 'red'});
    return;
  }
  showRenameLoading.value = true;
  try {
    await nodeService.renameFaceTrainPersonName({original_name: prevSelectedPersonName, new_name: name});
    await dataBindImages();
    await onLeftPersonMenuClicked(selectedPerson.value);
  } finally {
...
async function onAdNewSubmit() {
  showAdNewSubmitLoading.value = true;
  try {
    const result = await nodeService.newFaceTrainPerson({name: selectedPerson.value.name});
    if (result) {
      mode.value = Mode.Edit;
      await dataBindImages();
      await onLeftPersonMenuClicked(selectedPerson.value);
