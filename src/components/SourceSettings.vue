      ...
      <q-page padding style='background-color: whitesmoke;'>
        <div class='q-pa-md'>
          <q-btn-group class="gt-xs">
            ...
            <q-btn push :label="$t('snapshot_for_ai')" color='cyan' icon='auto_awesome' @click='step=4' :disable="!source.enabled" />
          ...
          <q-stepper v-model='step' vertical color='cyan' animated>
            ...
            <q-step id='step4' :name='4' :title="$t('snapshot_for_ai')" icon='auto_awesome' color='cyan' :done='step > 4'>
              <q-form class='q-gutter-md'>
...
function onFindOptimalSettings2() {
  let model = source.value;
  if (!model.name || !model.address) {
    $q.notify({
      message: t('v_enter_both_name_and_address'),
...
function onRecordVideoCodecChanged() {
  const arr = [3, 4, 15, 16];
  const isWebM = arr.includes(<any>source.value.record_video_codec);
  source.value.record_file_type = isWebM ? 1 : 0;
}

// function onStreamUriSelected(newUri: string) {
//   source.value.address = newUri;
// }

watch(() => source.value.address, (newValue: string) => {
  const isInsertMode = isNullOrEmpty(source.value.id);
  if (newValue && isInsertMode) {
    const isRtsp = newValue.toLowerCase().startsWith('rtsp');
    source.value.rtsp_transport = isRtsp ? 1 : 0; // 1: TCP, 0: Auto
    source.value.flv_player_type = isRtsp ? 0 : 1; // 0: MpegTsPlayer, 1: FlvPlayer
  }
