      ...
      <q-list bordered class="rounded-borders" style="max-width: 350px">
        ...
        <q-item v-for="item in list" v-ripple :key="item.id">
          <q-item-section avatar>
            <q-img :src="'data:image/jpg;base64, ' + item.base64_image" spinner-color='white' width="80px"
                   :style="{'cursor': item.type !== 3 ? 'pointer' : 'default'}"
                   @click="onOpenImageNewWindow(item)" />
          </q-item-section>
...
import { AiDto } from 'src/utils/models/ai_model';
import { myDateToJsDate, timeSince } from 'src/utils/utils';
...
onBeforeUnmount(() => {
  ...
  if (timeSinceInterval) {
    clearInterval(<any>timeSinceInterval);
  }
...
const setUpTimes = (model: any, type: number): NotificationViewModel => {
  const item: NotificationViewModel = { shown: popupShow.value, type };
...
async function addAiItem(responseModel: NotifierResponseEvent) {
  const aim: AiDto = JSON.parse(atob(responseModel.base_64_object));
  if (!aim || !aim.source_id) {
    return;
  }
  if (!(aim.source_id in cachedSources)) {
    await addCachedSource(aim.source_id);
  }
  const item: NotificationViewModel = setUpTimes(aim, 0);
  if (aim.detections && aim.detections.length) {
    const arr: string[] = [];
    for (const d of aim.detections) {
      arr.push(`${d.label}`);
    }
    if (arr.length > 0) {
...
function onOpenImageNewWindow(item: NotificationViewModel) {
  if (item.type === 3) return;
  const image = new Image();
  image.src = 'data:image/jpg;base64,' + item.base64_image;
